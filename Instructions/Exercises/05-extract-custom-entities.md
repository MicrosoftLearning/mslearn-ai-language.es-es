...
Además de otras capacidades de procesamiento de lenguaje natural, el servicio Lenguaje de Azure AI permite definir entidades personalizadas y extraer instancias de ellas desde el texto.
...
1. Use el botón **Crear un recurso**, busque *Lenguaje* y cree un recurso de **servicio de Lenguaje de Azure AI**. Cuando se le pregunte sobre *características adicionales*, seleccione **Custom text classification & extraction (Clasificación y extracción de texto personalizado)**. Cree el recurso con los valores siguientes:
...
3. En la cuenta de almacenamiento, seleccione **Configuración**, que se encuentra bajo **Configuración** y habilite la opción **Permitir el acceso anónimo de blobs** y, a continuación, seleccione **Guardar**.

4. Seleccione **Contenedores** en el menú de la izquierda, que se encuentra debajo de **Almacenamiento de datos**. En la pantalla que aparece, seleccione **+ Contenedor**. Denomine al contenedor `classifieds` y establezca el **Nivel de anónimo** en **Contenedor (acceso de lectura anónimo para contenedores y blobs)**.
...
5. Después de crear el contenedor, selecciónelo y haga clic en el botón **Cargar** para cargar los anuncios de ejemplo que ha descargado.

## Creación de un proyecto de reconocimiento de entidades con nombre personalizado

Ahora está listo para volver a establecer un proyecto de reconocimiento de entidades con nombre personalizado. Este proyecto proporciona un lugar de trabajo para compilar, entrenar e implementar el modelo.
...
1. Cree un proyecto nuevo con la siguiente configuración:
    - **Conectar almacenamiento**: *es probable que este valor ya esté rellenado. Cámbielo por su cuenta de almacenamiento si no está cambiado ya*
    - **Información básica:**
    - **Nombre**: `CustomEntityLab`
        - **Idioma principal del texto**: inglés (US)
        - **¿Su conjunto de datos incluye documentos que no están en el mismo lenguaje?** : *No*
...
1. En el lado derecho, en el panel **Actividad**, seleccione **Agregar entidad** y agregue una identidad nueva denominada `ItemForSale`.
1.  Repita el paso anterior para crear las siguientes entidades:
    - `Price`
    - `Location`
1. Después de crear las tres entidades, seleccione **Ad 1.txt** para que pueda leerla.
1. En *Ad 1.txt*: 
    1. Resalte el texto *face cord of firewood (cordón facial de leña)* y seleccione la entidad **ItemForSale** .
    1. Resalte el texto *fDenver, CO* y seleccione la entidad **Ubicación** .
...
1. Use el botón **Siguiente documento** para pasar al siguiente documento y continuar asignando texto a las entidades adecuadas para todo el conjunto de documentos, agréguelos todos al conjunto de datos de entrenamiento.
...
3. Seleccione la opción para entrenar un nuevo modelo denominado `ExtractAds`
...
Para probar las capacidades de extracción de entidades personalizadas del servicio Lenguaje de Azure AI, desarrollará una aplicación de consola sencilla en Visual Studio Code.
...
1. En Visual Studio Code, en el panel **Explorador**, vaya a la carpeta **Labfiles/05-custom-entity-recognition**, expanda la carpeta **CSharp** o **Python** según sus preferencias de lenguaje y la carpeta **custom-entities** que contiene. Cada carpeta contiene los archivos específicos del lenguaje de una aplicación en la que va a integrar la funcionalidad de clasificación de texto de Lenguaje de Azure AI.
1. Haga clic con el botón derecho en la carpeta **custom-entities** que contiene los archivos de código y abra un terminal integrado. A continuación, instale el paquete del SDK de Lenguaje de Azure AI Text Analytics mediante la ejecución del comando adecuado para sus preferencias de lenguaje:
...
1. En la función **Main (Principal)**, tenga en cuenta que el código existente lee todos los archivos de la carpeta **Artículos** y crea una lista con su contenido. En el caso del código de C#, se usa una lista de objetos **TextDocumentInput** para incluir el nombre de archivo como Id. y el lenguaje. En Python se usa una lista sencilla del contenido del texto.
1. Busque el comentario **Extraer identidades** y agregue el siguiente código:
